...
const LessonInfoModal = ({onClose, params, initialLessonData, onLessonUpdated}) => {
            ...
            .then(async (data) => {
                if (data && data.students) {
                    ...
                    data.students.forEach(st => {
                        initialAttendance[st.id] = st.present;
                    });
                    ...
                    if (!isMentor && params.studentId) {
                        try {
                            ...
                            console.error("Failed to load assignment submission history:", err);
                        }
                    }
                }
                setLoading(false);
    ...
    const handleCompleteLesson = async () => {
        ...
        try {
            const response = await axios.put(`http://localhost:8080/api/v1/lessons/${params.lessonId}/complete`, {
                title: lessonTitle,
                presentStudentIds,
                assignments
    ...
    return (
        <div className="p-4 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
            <div className="flex flex-col gap-1">
                <label className="font-semibold text-sm">Lesson Topic:</label>
                <input
                    type="text"
                    value={lessonTitle}
            ...
            <AssignmentList
                isCompleted={isCompleted} initialLessonData={initialLessonData} assignments={assignments}
                isMentor={isMentor} params={params} students={students}
                submittedAssignmentIds={submittedAssignmentIds} activeAssignmentId={activeAssignmentId}
                setActiveAssignmentId={setActiveAssignmentId}
                studentComment={studentComment} setStudentComment={setStudentComment} uploading={uploading}
            ...
            {!isCompleted && isMentor && (
                <div className="bg-gray-100 p-2 rounded flex flex-col gap-2 border">
                    <input
                        ...
                        className="border p-1 text-xs rounded bg-white w-full"
                    />
                    <button
                        type="button"
                        onClick={handleAddAssignment}
                        className="bg-blue-600 text-white text-xs px-2 py-1 rounded hover:bg-blue-700 self-end"
                    >
                        + Add Task
                    </button>
                </div>
            ...
            <div className="flex gap-2 mt-2">
                {!isCompleted && isMentor && (
                    <button
                        ...
                        className="bg-green-600 text-white font-medium py-2 px-4 rounded text-sm flex-1"
                    >
                        Save and Complete
                    </button>
                )}
                <button
                    onClick={onClose}
                    className="border border-gray-300 text-gray-700 font-medium py-2 px-4 rounded text-sm flex-1"
                >
                    Close
                </button>
            </div>
